...
public class FaqTests {
    ...
    @ParameterizedTest
    @MethodSource("questionsId")
    void faqListTest(int id, String answer) {
        driver.get("https://qa-scooter.praktikum-services.ru/");
        MainPage mainPage = new MainPage(driver);
        // возможно ожидание не нужно, если страница быстро грузится
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
        mainPage.acceptCookie();
        mainPage.clickFaqQuestionByNumber(id);
        mainPage.checkFaqAnswerByTextIsVisible(answer);
    }

    private static Stream<Arguments> questionsId() {
        return Stream.of(
